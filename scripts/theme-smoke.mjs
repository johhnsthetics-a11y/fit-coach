...
const nutritionRlsMigration = existsSync(nutritionRlsMigrationPath)
  ? readFileSync(nutritionRlsMigrationPath, 'utf8')
  : ''
...
const checks = [
  ...
  ['Service worker cache is versioned for multiple nutrition plans', sw.includes('coach-fit-pro-pwa-20260916-nutrition-multiple-v1')],
  ['Public and application versions identify the nutrition release', version.trim() === 'nutrition-multiple-plans-20260916' && app.includes(`COACH_FIT_PRO_BUILD_MARKER = '${version.trim()}'`)],
...
['Notification overlay supports close controls', app.includes('handleNotificationKeyDown') && app.includes('coach-notification-popover-backdrop') && app.includes('Fechar notificações')],
  ...
  ['Nutrition prescribed and student diet cards are theme-aware', app.includes('nutrition-plan-card-v6') && app.includes('nutrition-plan-meal-card-v6') && app.includes('plans={studentNutritionPlans}') && css.includes('nutrition-plan-list-theme-v6') && css.includes('.student-mobile-shell.app-theme-light .nutrition-plan-card-v6')],
  ['Patient diet list must not truncate phases to the first plan', !app.includes('studentNutritionPlans.slice(0, 1)')],
  ...
  ['Nutrition RLS migration is already applied or included', !nutritionRlsMigration || (api.includes('20260909_fix_nutrition_rls_policies.sql') && css.includes('nutrition-rls-professional-polish-v1'))],
  ['Nutrition RLS owns-student check is valid when migration is included', !nutritionRlsMigration || (nutritionRlsMigration.includes('security definer') && nutritionRlsMigration.includes('public.coachfit_owns_student(student_id)'))],
  ['Nutrition save uses RPC before REST fallback', api.includes("rpcRequest('save_nutrition_plan'") && api.includes('saveRemoteNutritionPlanViaRpc') && (!nutritionRlsMigration || nutritionRlsMigration.includes('create or replace function public.save_nutrition_plan(plan jsonb)'))],
