...
['Notification overlay supports close controls', app.includes('handleNotificationKeyDown') && app.includes('coach-notification-popover-backdrop') && app.includes('Fechar notificações')],
  ...
  ['Recoverable browser DOM errors do not trigger blocking app warning', app.includes('function isRecoverableDomMutationError') && app.includes('recoverableDomError') && main.includes('function isRecoverableDomMutationError') && main.includes('failed: true, recoverableDomError: isRecoverableDomMutationError(error)')],
