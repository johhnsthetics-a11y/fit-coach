-- Coach Fit Pro
-- Repair the nutrition save path that can surface PostgreSQL 42501.
-- Idempotent and scoped to nutrition_plans / nutrition_meals only.
...
revoke all on function public.coachfit_owns_student(uuid) from anon;
...
begin
  ...
  if v_plan_id is not null then
    ...
    if not found then
      raise exception 'Dieta nao encontrada para este treinador.' using errcode = '42501';
    end if;
...
revoke all on function public.save_nutrition_plan(jsonb) from anon;
